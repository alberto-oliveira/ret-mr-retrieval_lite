...
def scoring(votes, dists, invidx):


    # invidx indexes db features to their source image. The last feature is indexed to the
    # last image index. Thus, invidx[-1] + 1 is the number of db images
    dbsize = invidx[-1] + 1
    votescores = np.zeros(dbsize, dtype=np.float32)
    distscores = np.zeros(dbsize, dtype=np.float32)

    # n = # of features, k = # of neighbors
    n, k = votes.shape
...
def ranking(votescores, distscores, namearray):
...
def search_batch(batchfile, retcfg, outdir):
...
def search_single(qname, retcfg, outdir):
