...
Implements utility drawning functions.
...
def draw_matches(im1, keyps1, im2, keyps2, matches, draw_keypoints=True, mcolor=(255, 255, 255), mthickness=1):
    ...
    if draw_keypoints:
        im1keyp = cv2.drawKeypoints(im1, keyps1, color=(0, 0, 255), flags=cv2.DRAW_MATCHES_FLAGS_DEFAULT)
        im2keyp = cv2.drawKeypoints(im2, keyps2, color=(0, 0, 255), flags=cv2.DRAW_MATCHES_FLAGS_DEFAULT)
    else:
        im1keyp = im1
        im2keyp = im2
...
def draw_cluster_matches(im1, keyps1, mpts1, im2, keyps2, mpts2, clusters=[],
                         draw_keypoints=True, mthickness=1):
