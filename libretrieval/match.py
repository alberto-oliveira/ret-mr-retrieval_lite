...
Implements descriptor matching functions and utilities to manage OpenCV's DMatch class structures.
...
def match_descriptors(desc_a, desc_b, m_type="Brute", norm_type=cv2.NORM_L2, knn=1, matcher=[]):
    ...
    if not matcher:
        if m_type == "Brute":
            matcher = cv2.BFMatcher(normType=norm_type, crossCheck=True)
        elif m_type == "Flann":
            matcher = cv2.DescriptorMatcher_create("FlannBased")
        else:
            print "Unsuported matcher type! {0:s} not in ({1:s})".format(m_type, ", ".join(mtype_list))
            raise
...
def get_matched_keypoints(matches, qkeyp, tkeyp):
    ...
    dists = np.array(dists, dtye=np.float32)
